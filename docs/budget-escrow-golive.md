...
Do the whole checklist before any money goes in. The whole point is
to find a problem on a one-cent budget rather than a one.
...
## 4. Only then, point anything at it

- Put the address in the frontend config.
- Start with a low `maxPerDraw` and a short `deadline` on the first budget. Both can be set generously later; neither can be changed on a
  budget that already exists.

---

## If something is wrong after money is in
...
- Covered already. Contract logic, 33 tests including reentrancy on
  ...
  boundaries, and a solvency fuzz. These run against a BSC mainnet
  fork, which is closer to mainnet than testnet is.
- Covered by the checklist above. Deployment mechanics, constructor
 arguments, verification, wallet signing, gas, event emission.
