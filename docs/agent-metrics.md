...
Coverage now advances via a dedicated Render Background Worker (`backend/worker.py`, service `escrow-compat-audit-worker`) that loops the same batch auditor continuously, rather than the request-cycle-bound GitHub Actions step this doc originally described — that scheduled step was removed once the worker took over (see `full-registry-batch.yml`), since the worker isn't capped at ~120s per run and reaches full coverage far faster. The worker was live-verified processing ~50 real agents per 90-second batch, unattended, immediately after deploy.
