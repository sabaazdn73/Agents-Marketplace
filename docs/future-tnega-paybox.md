...
| `supported` (cached, 15 min) | `core/b402.py` | Live, returns 10 kinds, all BSC |
...
| `POST /api/paybox/sessions` → HTTP 402 | `backend/server.py` | Live-tested |
...
assets, verified on-chain (each address matched by calling `name()` on BSC mainnet and comparing it exactly to the name B402 returns, rather than trusting a well-known-address list): $U, USDT, USDC, USD1, all 18 decimals on BSC, which is a trap worth naming, since USDT/USDC use 6 decimals on Ethereum and getting it wrong would misprice a payment by 10¹².
...
- No settlement has been executed. `verify`/`settle` need a signed EIP-3009 or Permit2 authorization from a funded wallet. Producing one means handling a private key holding real $U, which is out of scope for this backend by design, it never holds a key. The verify path was exercised live with structurally-payloads and returns real, structured rejections (`invalid_exact_evm_payload_recipient_mismatch`); the settle path is code-complete and unexercised. Completing a payment end to end is the next step, and it needs a funded browser wallet, not more backend work.
...
### Two API details the guide's prose doesn't spell out

Both cost debugging time and are recorded so the next person doesn't repeat them:
...
- Ships a MCP integration: `pay --sandbox claude` wires Pay MCP tools straight into Claude Code, an agent framework gets paid-API access as a tool call, no separate payment integration to build.
...
## inspiration, multi-chain direction (future, larger scope)
