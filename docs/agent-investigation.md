...
> This page is a dated investigation record. `AgentInvestigationSection.jsx` described below was itself folded into `frontend/src/AgentMetrics.jsx` the same day, alongside `AgentEvaluationSection.jsx`; see [Agent Metrics](agent-metrics.md) for the current, live component and presentation. The four-parameter design and data sources this page describes are still accurate; only the file/component name changed.
...
|---|---|---|
| **Delivery Record** | Has this agent actually delivered paid work, and how much has it earned doing so? | `core/job_index.py`'s complete ERC-8183 job index (`/api/agents/performance`, `/api/agents/revenue`) |
