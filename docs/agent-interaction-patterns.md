...
1. Censused the full BSC dataset (`full_agent_registry`, chain_id 56, 64,821 agents, the same pool the live house diversifies from) by category, service reachability, and structured fields (`x402_supported`, `supported_protocols`) already present on each 8004scan record.
...
### The dominant fact: most of the house has no live service to interact with at all
...
| x402 pay-per-call, described as such by the agent itself | 3 reachable (+1 unreachable) | Rook Trading Intelligence (x402-primary, "72 x402 endpoints"), 2U2.ai Content Agent (dual x402 + job), stockanalyst-agent (dual ERC-8183 escrow + x402 async) |
| Dev/test/throwaway registrations (technically live, not a product) | ~9 | `api.bortagent.xyz`'s one-word entries ("premium", "ala", "alpha", "hook", "flap", "bort578", two "Untitled Agent"s, "deepliar"), all one developer's own iteration, all technically escrow-compatible |
...
- Session-key/delegation beyond the existing Altana flow: 1 match (`bnb-lp-quant.agent`, "runs...from client session keys"), and reading it, this described exactly the Altana session pattern the house's hire flow supported at the time (that path was later removed 2026-09-03, see [Known Limitations](limitations.md#altana-passkey-session-hiring-removed-2026-09-03)), a confirmation the flow as it then existed covered it, not a gap.
...
## The resulting design: three additions, not a rebuild
...
- `frontend/src/agentEvaluation.js`: a new `PRIMARY_CTA.HIRE_CAUTION` state for auth-gated agents, and pass-through of `differentProtocol`/`offersX402Alternative` for the UI layer to render.
...
- `frontend/src/EscrowCompatibilityWarning.jsx`: `useHireFlowEscrowGate` (the last-chance funding-modal gate) gained the same non-blocking auth-gated caution, right before money moves, consistent with the explicit "default to the most cautious action" instruction, without a hard checkbox gate on evidence this project has always treated as inconclusive rather than a confirmed failure.
