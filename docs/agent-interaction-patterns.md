...
- **Session-key/delegation beyond the existing Altana flow**: 1 match (`bnb-lp-quant.agent`, "runs...from client session keys"), and reading it, this describes exactly the Altana session pattern the marketplace **already supports**, a confirmation the existing flow covers it, not a gap.
