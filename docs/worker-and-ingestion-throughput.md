...
This section originally disclosed, without fixing, that ingestion now runs far faster than `full_registry_analysis.py`'s own downstream pass, meaning the ingested-but-unanalyzed backlog would grow faster than before. Confirmed as a real, severe capacity mismatch and fixed the same day, once the user made it an explicit, non-negotiable requirement — see [Full BSC Registry Analysis](full-registry-analysis.md#evaluation-coverage-guarantee--every-ingested-agent-gets-fully-evaluated-in-bounded-real-time-2026-08-29) for the full real capacity math, the bounded-backlog pause/resume mechanism now enforcing this, and the live-verified proof it engages correctly.
