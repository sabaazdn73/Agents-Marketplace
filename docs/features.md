...
- Owner wallet BNB balance, with its US dollar value alongside it: a 30-minute average price from the PancakeSwap v3 WBNB/USDT pool, read on chain and labelled "USD via USDT (BSC-USD)".
...
Nine pre-built, fork-tested Skills from Altana's public registry (PancakeSwap trading/liquidity, Venus/Aave lending, Lista staking, four.meme, copy-trade detection, wallet tracking, x402 payments), no building required. The registry's tenth, Token Radar (trending pools), was removed on 2026-09-25, because its data source's terms do not clearly allow a commercial site; see [Deferred work](deferred.md). Read-only/detection Skills need no wallet at all. For a transaction Skill, the user's own already-connected wallet (MetaMask, etc.) signs each run directly, one signature per run, no separate funding needed. See [Direct-Wallet Skill Execution](direct-wallet-skills.md).
...
- Task 1, security/trading risk check: complete. A Token Radar skill lookup (0.216s, $0; the skill has since been removed from the site) versus the same manual DexScreener check by hand (2m 00s); the agent caught a red flag (98x volume-to-liquidity) in a fraction of the time.
