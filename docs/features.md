...
- **Trading Agent** (live): real spot buys of any BSC token through PancakeSwap's own router, same direct connected-wallet execution and entry fee as Staking. Real, evidence-based risk signals shown before signing, computed entirely from on-chain reads (no external API): price impact (the realized rate at the requested trade size vs. a small reference trade along the same route) and liquidity depth (trade size as a % of the pool's real, current USDT reserve); either crossing a real threshold requires an explicit acknowledgment before continuing, same pattern as the escrow-compatibility gate. See `pancakeswapSkill.js`'s own "Trading Agent" section for the real feasibility check confirming this stays inside the simple, direct-wallet, single-transaction pattern, unlike Avantis perpetuals below.
