...
- **Universal search fallback** — pasting a real agent id, wallet address, or contract address that the local name search doesn't match triggers a real, live check against the authoritative source (8004scan live, or a direct on-chain read) instead of a dead "no results". Always a real, honest, categorized answer — a found-but-uncurated agent, a plain wallet's real balance, an identified token/contract, or a genuine "doesn't exist" — never silence. See [Universal Search Fallback](universal-search.md).
