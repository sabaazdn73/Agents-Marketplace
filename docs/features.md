...
- **Perpetuals Agent**: Coming Soon. Investigated (2026-09-03): Avantis (Base) trades require a live call to their own off-chain calldata-builder API (`tx-builder.avantisfi.com`) and settle asynchronously — a signed order fills "usually within seconds" or genuinely expires unfilled after ~15-30s, confirmed directly against Avantis's own docs and Base's official MCP plugin docs. A materially different, more complex integration shape (a live third-party API in the critical path, real fill-or-expire uncertainty) than every other Native Agent here; not built. A read-only dashboard of the user's own existing positions remains a real, smaller, honestly-scoped option if wanted later. (The earlier "no BSC bridge" reasoning was stale — Base is reachable via a standard wallet chain-switch — and is superseded by this finding.)
...
- **Web2 Agents + PayBox**: Coming Soon, a vision/roadmap card only, no code behind it. Bridges Web2 AI shopping/commerce agents, like Anthropic's open-source Commerce Agents blueprint (which explicitly stops at cart handoff, never completing payment itself), to on-chain settlement via a future Tnega PayBox, MoonPay's confirmed direct BSC support as the near-term rail. A genuinely separate, much larger second idea, a "describe an agent in a prompt, get one built and wired to payment automatically" platform, is its own future project, not scoped here. See [Future: Tnega PayBox](future-tnega-paybox.md) for the full research.
