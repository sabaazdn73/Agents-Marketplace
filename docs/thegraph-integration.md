...
- The first version found the highest stored agent id by sorting `token_id` descending. `token_id` is stored as a string, so lexical order puts `"99979"` above `"332377"`. It reported a maximum of 99,979 when the one was 332,377 and would have re-fetched a third of the registry on every run. Now computed with an aggregation that converts to a number first.
