...
Tnega has three real, separately-deployed pieces: a React frontend (one codebase, two responsive apps), a FastAPI backend, and a MongoDB store. A fourth service — a real "explainer agent" listed in the marketplace itself — is an independently-deployed ERC-8004/ERC-8183 seller agent used to demonstrate the real hire flow end to end.
...
`App.jsx` picks between them at a 768px breakpoint (`window.innerWidth`), and also handles a small set of standalone routes outside the tab structure — `/ecosystem`, `/status`, `/data-sources`, `/partners`, `/docs` — via a plain `pathname` check (no router library; see [Features](features.md) for what each page does). These are genuinely separate, deep-linkable URLs, not tabs. The main app's own tabs (`/market`, `/my-agents`, `/report`, `/learn`, `/build`, `/sell`) get real URLs the same way.
