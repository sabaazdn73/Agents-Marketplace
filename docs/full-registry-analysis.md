...
Precise, and deliberately narrow: an agent whose on-chain `tokenURI` is empty, or whose resolved metadata has no `services[]` entry with an http(s) endpoint (`service_status == "no_endpoint"`, the same real state `core/agent_health.py`'s health-check already computes) is permanently unreachable by design — there's no real path to ever hiring or interacting with it. Keeping it costs real storage for zero real benefit, so it's deleted the moment this is confirmed, not stored.
...
**Correction, same day: this policy is BSC-only, not universal, and was briefly applied incorrectly to every chain.** `core/agent_health.py`'s health-check reads an agent's `tokenURI` via one hardcoded contract address (`IDENTITY_REGISTRY`) over one BSC-only RPC, with no per-chain awareness at all. For a non-BSC agent this reads BSC's own registry for that agent's real token_id, which almost never exists there, so the check comes back `no_endpoint` for reasons that have nothing to do with whether the agent's own chain has a real endpoint — a confirmed false-positive machine for every chain except BSC, not a partial signal.
...
**Live-verified recovery in progress**, both from manually triggered batches and the already-running background worker picking up the same reset checkpoint on its own real schedule:
...
A full live diagnostic pass, prompted by a report that the agent count had stopped growing. Real numbers, all measured live.
...
**8004scan deep-offset degradation is real and ongoing.** Live-checked offsets 700,000 / 806,000 / 808,000: all three timed out, while a shallow offset-0 call returned in 1.1s. This is what fills `full_registry_skipped_offsets` (361 offsets, ~36,100 agents' worth). That collection is retried automatically at the head of every ingest batch (`retry_skipped_offsets`), so the design is already correct — the blocker is upstream availability, not our handling of it.

**Storage has real headroom again.** 258.9MB visible of the 512MB Atlas quota, and a real write probe succeeded. The quota problem from 2026-09-11's cleanup is not currently recurring.
