...
Real, precise, and deliberately narrow: an agent whose on-chain `tokenURI` is empty, or whose resolved metadata has no `services[]` entry with an http(s) endpoint (`service_status == "no_endpoint"`, the same real state `core/agent_health.py`'s health-check already computes) is permanently unreachable by design — there's no real path to ever hiring or interacting with it. Keeping it costs real storage for zero real benefit, so it's deleted the moment this is confirmed, not stored.

**What this does NOT touch**, stated explicitly since the line matters: an agent that's SaaS-incompatible, different-protocol, auth-gated, `not_responding` right now, or `unknown` (this pipeline's own metadata-resolution failure, not a confirmed fact about the agent) keeps its real data. Those represent real, live, responding businesses or services this project simply can't hire through today, or agents this pipeline hasn't been able to check yet — deleting either would be premature and information-destroying, not a safe cleanup. Only the unambiguous, confirmed case is covered.
...
**Three of the five extra chains had never been scheduled at all.** Monad (3,200/10,154), Billions (900/25,966) and Celo (2,000/9,673) last ingested 24.5 hours ago — the exact moment they were last triggered by hand — and had received zero automated runs since. Cause: the workflow step calling `/api/admin/multichain-registry-batch` was written when those chains were added but never landed, because this session's GitHub token lacks the `workflow` OAuth scope needed to push `.github/workflows/*`. The deployed workflow only ever called five endpoints. Robinhood (32/32) and Arbitrum (1,371/1,371) are genuinely complete.
