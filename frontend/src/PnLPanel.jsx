...
// docstring for the full methodology (Zerion balance charts at
// job start/end, minus gas costs). Shared verbatim by web and mobile.
//
// Deliberately opt-in (a button, not auto-fetched), same reason as
// WalletPortfolioPanel.jsx: Zerion's real, measured tier is 300
// requests/day, and this can cost several requests per agent (a
// chart + a fee lookup per qualifying job, up to
// MAX_JOBS_PER_SUMMARY of them), only spent when someone opens
// this agent's detail page and asks.
//
// Real, deliberate label, per the explicit requirement this was
// built against: this is "Live/Forward-tested PnL, measured from real
// on-chain balances during an hire", a creator's own claimed
...
// The one, category group PnL is a coherent question for, reuses
// categoryGroups.js's own grouping (never a separately-hardcoded
...
function JobRow({ job }) {
 ...
 Job #{job.job_id} · real {job.start_value_usd != null ? `$${job.start_value_usd.toFixed(2)}` : 'n/a'} → {job.end_value_usd != null ? `$${job.end_value_usd.toFixed(2)}` : 'n/a'}
...
export default function PnLPanel({ ownerAddress, category }) {
 ...
 // category where PnL isn't a coherent question, no button, no
  // placeholder, nothing to click on by mistake. The backend re-checks
  // this same eligibility independently (never trusts this
 ...
 <Loader2 size={12} className="animate-spin" /> Comparing on-chain balances…
      </div>
    );
  }
 ...
 <span>Couldn't check PnL right now ({state.message}).</span>
  ...
  const { data } = state;
  if (!data.applicable) {
 // Real, edge case: the backend's own eligibility check
 ...
 Live/forward-tested PnL, measured from on-chain balances during {data.jobs.filter((j) => j.available).length > 1 ? 'hires' : 'an hire'},
