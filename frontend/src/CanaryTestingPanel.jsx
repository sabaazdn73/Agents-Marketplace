...
// methodology.md for the full design and reasoning. Deliberately NOT
...
// analysis.md).
//
// REAL, DELIBERATE SAFETY DESIGN: every canary hire here goes through
// the EXACT SAME real, client-side hire flow (useHireAgent.js) as any
// other hire in this product, the button below triggers a wallet
...
// bug found and fixed (2026-08-27): this fetch had NO client-side
// timeout at all, same structural gap already found once this
...
// at ~4-7s even after fixing get_stored_agents()'s own O(k^2)
// clustering slowdown (see core/clustering.py). Without a timeout, any
// connection stall (a real, observed failure mode on this free-tier
// host, same as the AgentPerformance investigation found) left
// `candidates` stuck at its initial `null` forever, rendering the
// "Loading candidates…" spinner indefinitely, exactly the real
// symptom reported. Fixed with the same timeout + retry pattern.
...
function CandidateRow({ agent, budget, onDone }) {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setResult({ ok: true, message: `Job #${newJobId} funded and logged as a canary test.`, jobId: newJobId.toString() });
      onDone?.();
      if (rec.over_cap) {
 setResult((r) => ({ ...r, message: r.message + ' (Note: this pushed spend over the weekly cap, pause before running more.)' }));
      }
    } catch (e) {
 ...
 title={overCap ? 'Would exceed the weekly canary budget cap' : `Fund a real ${DEFAULT_TEST_BUDGET} $U test job, your connected wallet will prompt you to sign`}
        >
