...
// first data-consuming CoinGecko integration; previously CoinGecko
// was only used for a /status reachability check, nothing displayed real
// data from it). Shared by web + mobile so both compute the same USD
// context next to an agent's owner-wallet BNB balance.
import { useEffect, useState } from 'react';

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000';
let _cached = null; // one fetch per page session, not per component instance
...
export function useBnbPrice() {
 ...
 .catch(() => { /* failure, stays null, no fabricated price shown */ });
    return () => { cancelled = true; };
  }, []);

 return usd; // number, or null (never a fabricated placeholder)
}
