...
// the full investigation this was built from: live protocol probes
...
// NOT category. Every category group contains both escrow-compatible
// and SaaS/off-chain-incompatible agents, there's no clean category
// boundary. What determines how an agent should be evaluated and
...
// investigation (every category group including Unclassified, a full
// census of the reachable population, live multi-format probing, ten
// hypothesized patterns explicitly checked against the data). Found
// the plain escrow-compatible/incompatible binary was hiding two more
// real, distinct, evidence-backed states, a auth-gate (previously
// indistinguishable from "just works") and a real "different protocol,
// not broken" case, plus a small, real, additive x402 signal.
...
// explicitly checked and found to have zero or near-zero real
...
export const PRIMARY_CTA = {
  HIRE: 'hire',
 // Real, new (2026-08-28), an agent whose probe result was
 // genuinely inconclusive because of a real 401/403, not a hard
 // rejection. "Hire" is still the real, primary action (there's
 ...
 * same probe result (core/protocol_compat.py's own additive fields,
...
export function evaluateAgent({ escrowIncompatible, authGated, differentProtocol, offersX402Alternative, category }) {
 ...
 // Trading & DeFi showed a need for these panels. Doesn't
