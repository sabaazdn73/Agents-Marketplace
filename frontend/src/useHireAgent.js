...
// checklist in the UI (StepChecklist.jsx), the exact same state
// this hook already tracked internally, just exposed instead of thrown
// away. bug fixed here too: `step` used to be overwritten with the
// literal string 'error' on failure, which lost which of the 5 steps was
// active when it failed, exactly the ambiguity a checklist is
// supposed to resolve. Now `step` always stays the last-active step
...
//
// - Real, confirmed library support: this project's installed
...
// jobId is only known AFTER its receipt is mined and decoded
//   (decodeJobIdFromReceipt below), but registerJob/setBudget/approve/
//   fund all REQUIRE that jobId as an argument. EIP-5792 batches are
...
//   jobCounter()=56665, getJob(56665) is a existing job and
...
//   WRONG job, a correctness/safety risk, not a hypothetical one.
...
// jobId is confirmed on-chain before anything that depends on it
//   is even constructed.
// - What IS genuinely safe to batch: registerJob + setBudget + (approve,
// if needed) + fund all take the SAME already-confirmed jobId and
// have no inter-dependency this batch's own atomicity doesn't
//   already resolve (an EIP-5792 batch with atomic support of 'supported'
// or 'ready' either lands as one on-chain unit or none of it does
//, see wallet_getCapabilities' atomic.status field, checked below,
// never assumed). So the real, reduction this flow offers is 2
// signatures (createJob, then one batch for the remaining on-chain
 ...
 * wallet's own wallet_getCapabilities response reports atomic batch
 * support for BSC ('supported' or 'ready' per EIP-5792's own status
 * values), 'unsupported' for every other outcome (capability query
...
export function useBatchHireCapability() {
 ...
 // Real, confirmed bug fixed 2026-09-02, found by re-checking this
        ...
        // already-unwrapped object, which is never a key on it, so
        // atomicStatus was unconditionally undefined regardless of what
 // the wallet reported. Every "unsupported" result this
 ...
 // Real, fallback: a wallet that doesn't implement
...
// Generic quality terms sent with every negotiate attempt. Deliberately
...
// structured terms to sign a quote against; the work spec it reads
...
// "unconfirmed", not silently hung forever, money is on the line here.
// waitForTransactionReceipt's own default (unbounded) previously meant a
// dropped-by-the-wallet transaction (never broadcast, so no node
// will ever produce a receipt for it) just left the UI stuck on a spinner
// with no signal to the user. 90s comfortably clears normal BSC
// confirmation time (~3 blocks) with margin.
const RECEIPT_TIMEOUT_MS = 90_000;

// The real, ordered step keys, matches HIRE_STEP_COPY below and the
// execution order in `hire()`. 'approving' is genuinely conditional
...
// was never telling the seller to start work (see
...
// negotiates fresh at execution time (a quote has a TTL; reusing a
...
export function useAgentQuote(agent) {
 ...
 // decimals for the settlement token, never assumed.
...
export function useHireAgent() {
 ...
 const [notifySkipReason, setNotifySkipReason] = useState(null); // reason the 'notifying' step didn't complete, if any

  // Wraps a write + receipt-wait so a step that never reaches the network
 // (wallet-side drop, the exact failure mode this was built for)
 // reports itself honestly instead of leaving the caller unsure whether to
  // retry (and risk a duplicate) or wait. Records the hash against
  // `stepKey` and marks it complete only once a receipt confirms it.
 ...
 // guard, added 2026-08-17: previously nothing checked the wallet's
 // active chain before starting a 5-step, real-money sequence.
 ...
 // if the wallet is sitting on a different network, which is
    ...
    try {
      // Read the settlement asset and its decimals, never
 ...
 // few seconds spent finding out negotiate isn't available.
      setStep('negotiating');
 ...
 // Step 1: create the job (client -> provider, on-chain tx)
      //
 // bug fixed 2026-08-19: expiredAt used to be just
 ...
 // for exactly this). OptimisticPolicy's disputeWindow is 7 DAYS
      //, so every job created with the old formula was already past its
 // submission deadline the moment it was created, regardless of
      ...
      // attempted was rejected because the cutoff had already passed
      // at creation time, read live and confirmed after the fact.
      //
      // Fix: read the disputeWindow and pad for it, so `expiryMinutes`
 // means what it looks like it means, usable processing time
      ...
      // The jobId comes from the emitted event/return value, decoded
 ...
 // Step 4: approve the settlement asset if needed (allowance
 ...
 // Step 6: tell the seller agent to start work. Real, confirmed gap
      ...
      try {
 // fix, 2026-08-26: job #56659's real, live rejection
 ...
 // bug, not a seller-side convention. Confirmed against the real
 ...
 // success_criteria check ever runs. There is no real, reliable signal
        ...
        // means to: a negotiation succeeded, meaning this seller speaks
 ...
 // just assumed, read our own explainer-agent's notify_funded
 ...
 // Real, non-fatal: same treatment as a negotiate failure, most
      ...
      // that's the step the checklist should mark as errored.
 ...
 // Real "sign once for the remaining steps" alternative, see this file's
  // own top-of-file note for the full investigation (why createJob
 ...
 // go into ONE wallet_sendCalls batch instead of up to 4 separate
 ...
 // that ignores that guard gets whatever error the wallet itself
 ...
 // Step 1: createJob, real, individually-signed and confirmed, same
      // as hire(). Kept separate on purpose: its jobId (decoded from
 ...
 // Step 2: ONE batch, registerJob + setBudget + (approve, if the
 // real, current allowance doesn't already cover it) + fund. All four
 // take the now-confirmed jobId; none of them needs a value only
 // available after this batch itself starts executing, so a real
      ...
      if (batchResult.status !== 'success') {
        throw new Error(
 `The batched steps didn't confirm successfully (status: ${batchResult.status}). ` +
 `Check this batch's status before trying again, it may have partially landed depending on ` +
 `your wallet's own atomicity guarantee: https://bscscan.com/tx/${batchResult.receipts?.[batchResult.receipts.length - 1]?.transactionHash || ''}`
        );
      }
 // tx hashes, one per call in the batch, in the same order,
 // recorded against each step key so the checklist can still
 ...
 // Step 3: same real, best-effort notify as the step-by-step path.
  ...
  const getRealJobStatus = useCallback(async (jobIdToCheck) => {
    ...
    // job is the on-chain struct: [id, client, provider, evaluator,
...
// transaction does, no vaguer marketing language. Shared by web
 ...
 * number they're approving/spending, not a generic amount. */
...
// as one wallet interaction instead of up to 4 separate ones. The
// per-step hashes are still recorded in stepHashes (see hireBatched
// above) for anyone who wants them, but StepChecklist only shows one hash
// per row, so this row links to the funding call specifically, the
 ...
 * contract, same state shape from useHireAgent, just mapped onto the
...
export function buildBatchHireStepList({ step, completedSteps, skippedSteps, stepHashes, error, budgetUnits, notifySkipReason }) {
  const amount = budgetUnits != null ? `${budgetUnits} ` : '';
 // The four sub-steps the batch covers are complete only once ALL of
...
// Decodes the jobId from the createJob transaction receipt. The
...
// a transaction before being trusted, flagged here rather than
// silently assumed correct, the same discipline used throughout this
// project.
function decodeJobIdFromReceipt(receipt) {
  // TODO: verify against a createJob call on BSC mainnet. If this
  ...
  // Confirm the event name/shape before relying on this in production.
