...
// param (the window an agent has to submit before the buyer can
...
// actions with no delivery/waiting period, there's no deadline
...
// Real, floor: this project's own one measured delivery-time
// data point (jobTiming.js's explainer-agent entry, ~60s) is nowhere near
// this, 15 minutes leaves margin for negotiation, network latency,
...
// ceiling, anchored to the same on-chain scale this flow
// already uses elsewhere: OptimisticPolicy's own disputeWindow (7 days,
// read live in useHireAgent.js) is the order-of-magnitude a buyer
...
// gets the exact same behavior this flow already had.
...
/** validation against the bounds above, returns a plain,
 * error string, or null when the value is fine. Never silently
...
export function validateDeadlineMinutes(minutes) {
 ...
 return `minimum: ${DEADLINE_MIN_MINUTES} minutes, anything shorter gives no agent a fair chance to deliver.`;
  }
  if (n > DEADLINE_MAX_MINUTES) {
 return `maximum: ${formatDeadline(DEADLINE_MAX_MINUTES)}.`;
  }
  return null;
}
