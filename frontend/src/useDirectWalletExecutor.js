...
// needs its OWN funding before anything can run. Real, value of the
...
// is real, avoidable friction, not a safety or technical requirement.
//
...
// direct-wallet hire path already depends on in production, atomic
// batching via wallet_sendCalls when the connected wallet supports it
// (same wallet_getCapabilities check as useBatchHireCapability
// there), sequential signing as an fallback when it doesn't.
...
// publicClient, execute(calls) }), so every Skill function in
...
// alternative way to GET one, not a fork of the skills themselves.
//
// Real, limitation, surfaced in the UI, not hidden: a directly-
...
// no further signature) genuinely doesn't apply here. Every run
// needs its own signature, right then, structurally the same
...
/** Real, live capability check, same wallet_getCapabilities call
...
/**
 * hook, returns null while no wallet is connected (the caller
 * should prompt connection, e.g. via RainbowKit's own ConnectButton,
 * rather than this hook doing it), or a executor once one is.
 */
export function useDirectWalletExecutor() {
  ...
  return {
    ...
    execute: async (calls) => {
      ...
      if (atomicSupported) {
 // Real, atomic batch, one signature covers every call
        ...
        if (result.status !== 'success') {
          throw new Error(
 `The batched steps didn't confirm successfully (status: ${result.status}). ` +
 `Check this batch's status before trying again.`
          );
        }
        return {
 ...
 // Real, sequential fallback, most connected wallets don't
 // support EIP-5792 batching yet. Each call is its OWN,
      // separately-signed transaction, no atomicity guarantee (if a
      // later call fails, earlier ones already landed on-chain), same
 // tradeoff any normal "approve, then act" dApp flow already
      // has. Never silently claimed to be atomic when it isn't.
      const transactionHashes = [];
      for (const call of calls) {
 // bug fixed 2026-09-01, found while wiring up the native
 // Staking agent (a payable call, BNB value attached): this
        ...
        // instead of the amount. Lista's existing listaStake() call
