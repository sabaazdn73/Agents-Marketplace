...
// for the full methodology. Deliberately INDEPENDENT of PnLPanel.jsx
// (which only ever looks at jobs hired through Tnega's own Altana-
// session flow): this one looks directly at the agent's own real,
// on-chain execution history, trades/rebalances/deposits it has
...
// agent ever traded, anywhere" vs "how did a Tnega-hired session
// specifically do"). Shared verbatim by web and mobile.
//
// Deliberately opt-in (a button, not auto-fetched), same reason as
// PnLPanel.jsx/WalletPortfolioPanel.jsx: Zerion's real, measured tier is
// 300 requests/day, and a single check here can cost up to 3 real
// Zerion calls (activity + portfolio + chart), only spent when someone
// opens this agent's detail page and asks.
//
// Real, attribution-confidence labeling, always shown, see
...
export default function OnchainPerformancePanel({ ownerAddress, category }) {
 ...
 // Real, same gating pattern as PnLPanel.jsx, this section doesn't
  ...
  // re-checks this same eligibility, never trusted from the client
  // alone.
 ...
 <Loader2 size={12} className="animate-spin" /> Scanning this agent's own on-chain history…
      </div>
    );
  }
 ...
 <span>Couldn't check on-chain history right now ({state.message}).</span>
 ...
 {data.window_capped_at_50_most_recent_transactions && ' (based on the most recent 50 transactions on this wallet, it may have history beyond that.)'}
      </p>
        ...
        <div className="mb-2 p-2 rounded-lg bg-white/60 dark:bg-white/5 border border-indigo-100 dark:border-indigo-500/10">
          ...
          {data.defi_positions.map((p, i) => (
            <div key={i} className="flex items-center justify-between text-[11px]">
              ...
              <span className="font-mono text-gray-500 dark:text-gray-400">{p.usd_value != null ? `$${p.usd_value.toLocaleString(undefined, { maximumFractionDigits: 2 })}` : 'n/a'}</span>
            </div>
          ))}
        </div>
      )}
 ...
 <p className="text-[10px] text-gray-400">{data.pnl_reason || "Found activity, but couldn't compute a value change for it yet."}</p>
        </div>
      )}

 {/* Real, independent second PnL signal (2026-08-30), Zerion's own
          dedicated FIFO cost-basis calculation over the same window,
          deliberately shown SEPARATE from the number above rather than
 replacing it: the two use genuinely different methodologies
 and can legitimately disagree for a wallet with in-window
 trading, showing both, clearly labeled, is more than
          picking one. */}
