// erc8183Negotiate.js
//
// client-side counterpart to backend/core/erc8183_negotiate.py.
//
// root cause this exists for (2026-08-21/22): job #56636 was funded
...
// Traced through the SDK verification logic
...
// OPTIONS preflight returned 405 with no Access-Control-Allow-Origin,
...
// negotiation_hash correctly recovers the agent's signer address via
...
/** call to our backend's negotiate proxy. Returns the raw negotiation
 * result on a accepted quote, or null, NEVER throws. The caller
 ...
 * ALONE, which is genuinely ambiguous whenever one owner operates
 * more than one registered agent (confirmed live: 1,457 owners
 * currently do), a hire could silently negotiate against a
 * completely different, wrong agent that just happens to share an owner.
 * Always pass it when the caller has it (every caller here does). */
...
/** call to our backend's notify_funded proxy, tells a strict ERC-8183
 * seller "I funded job X, please deliver" right after the on-chain
...
/** EIP-712 "notify_funded" authorization envelope for sellers that
 ...
 * project's own source, buyer-client/src/notify-auth.ts (the
 ...
 * this marketplace has no portfolio/risk data to honestly sign over.
 *
 * `signTypedDataAsync` is wagmi's `useSignTypedData()` return value, only
 * the connected wallet that created/funded the job can produce a
 * signature the seller's expected_client check accepts (it recovers the
 * signer and compares against the job's on-chain `client` address). */
...
/** port of build_job_description(negotiation_result), the exact
...
export function buildJobDescription(negotiationResult) {
 ...
 // the schema, never nested in `terms` (matches job #56620's real,
 ...
 * checks the funded budget against (must be >= this). */
