...
// every token an agent's owner wallet holds on BSC, with USD
...
// those requests when someone opens one specific agent's detail
...
//
// Real "does this agent's owner wallet do what it claims" signal
...
// this signal only renders where a DeFi position would be
// relevant evidence. Costs ZERO extra Zerion requests, the SAME
// no_filter positions call already returns DeFi/protocol positions
// (position_type != "wallet", protocol name) alongside plain token
// balances; this is purely a different way of presenting data already
// fetched for the button above, confirmed by reading the response
// shape, not assumed.
//
// Verified against agents already in this marketplace, not synthetic
// data: of 17 owner wallets across the four DeFi categories, 16 had
// zero DeFi positions and one (positioncrew-yield-optimizer.agent, a
// real "Yield Optimisation" agent) genuinely held Venus Protocol
...
// logo URL, fix already applied there, see that file's comment) so
...
// signal is only meaningful where a DeFi position would be
...
/** Groups the DeFi/protocol positions (position_type != "wallet") by
 * their protocol name, e.g. {"Venus": ["deposit","loan","reward"]}. */
...
function DefiMatchSignal({ positions }) {
 ...
 <span className="text-[11px] text-emerald-800 dark:text-emerald-300">This agent's owner wallet holds DeFi positions: {summary}.</span>
    </div>
  );
}
...
export default function WalletPortfolioPanel({ ownerAddress, category }) {
  ...
  if (state.status === 'idle') {
    return (
      ...
      >
 <Wallet size={12} /> {isDefiCategory ? 'Check wallet activity' : 'Check full wallet portfolio'} <ChevronDown size={12} />
      </button>
    );
  }
          ...
          {positions.map((p, i) => (
            <div key={i} className="flex items-center justify-between text-[11px]">
              ...
              <span className="font-mono text-gray-500 dark:text-gray-400">
                {p.usd_value != null ? `$${p.usd_value.toLocaleString(undefined, { maximumFractionDigits: 2 })}` : 'n/a'}
              </span>
            </div>
          ))}
        </div>
