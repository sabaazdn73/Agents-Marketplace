...
// a Infura BSC endpoint (bsc-mainnet.infura.io) as an automatic backup
...
// 2026-08-29 bug comment for the incident this caused (a direct-
...
// that transport now lives, so both callers get the same failover,
...
// version follows. Infura only gets a request when bloXroute's own
// attempt genuinely fails (a timeout, network error, or malformed/
 ...
 * a missing key never breaks anything, it just means no backup
 * exists yet. With one configured, bloXroute is still always tried
 * first; Infura only ever engages on a primary failure. */
