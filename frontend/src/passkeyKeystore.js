...
//
// investigation (confirmed against the installed @altananetwork/sdk,
// not assumed): the KeyStore contract exposes getKeys(user) and
...
// empirically by generating one key of each type and comparing bytes.
...
// other. Given the on-chain-read public key bytes, curve-membership
...
// their public key happens to validate on. Empirically verified: 200
// P-256 keys tested against secp256k1 point-validation, 0 false
// positives.
//
...
// KeyStore contract on BSC mainnet, read live from the SDK's own public
...
/** curve-membership check on SEC1-uncompressed public key bytes. */
...
/**
 * on-chain check: does `walletAddress` have a passkey (P-256) admin key
 * registered in the KeyStore contract? Every step is a live contract
