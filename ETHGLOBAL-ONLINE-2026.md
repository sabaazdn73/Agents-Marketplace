...
**Window:** from **17:00 Lisbon (16:00 UTC), 4 September 2026** — the moment ETHGlobal Online opened — to the present.
...
A proper audit followed: every field's identifier grepped across every `.js`/`.jsx` file in both apps. `owner_address` alone is consumed in **21 files**. Of 31 fields exactly **one** (`last_seen_at`, worth 0.55 MB) has no frontend consumer — field-trimming was simply exhausted as an approach.
