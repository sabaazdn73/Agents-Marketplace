...
one process, not three separate $7/month services:
  ...
  1. Escrow-compatibility audit (core/escrow_compat_audit.run_audit_batch)
     ...
     audit's purpose (warning a buyer before funding a job with an
  ...
  3. Full-registry analysis, health-check + categorization
     ...
     section for the full mapping of which evaluation steps apply to
     every ingested agent, the capacity math behind this loop, and
...
one asyncio event loop with no resource contention on the worker's
...
# IPFS gateway (ipfs.io), a genuinely different concern from
...
# the guarantee this loop exists for (see INGEST_BACKLOG_PAUSE_
# THRESHOLD below, the load-bearing mechanism), flagged in docs
...
# ── Full-registry ingestion loop, new 2026-08-29, concurrent
# fetching (see core/full_registry_ingest.py's own INGEST_CONCURRENCY).
#
# Real, bounded-backlog guarantee: this is the mechanism behind
    ...
    while True:
        try:
            ...
            if not paused and backlog >= INGEST_BACKLOG_PAUSE_THRESHOLD:
                paused = True
                log.warning(
                    "[ingest] pausing: unanalyzed backlog is %d (>= %d), waiting for the analysis loop "
            ...
            if paused:
                if backlog <= INGEST_BACKLOG_RESUME_THRESHOLD:
                    paused = False
                    log.info("[ingest] resuming: unanalyzed backlog dropped to %d (<= %d).",
