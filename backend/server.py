...
async def agent_detail(agent_id: str):
    ...
    what the list renders; the detail panel calls this when a human
    actually opens one specific agent -- a real, naturally-bounded,
    human-paced trigger, the same reasoning already applied to
    /quality-center and /contract-verification.

    Deliberately registered LAST in this file: FastAPI matches routes in
    registration order, so every literal /api/agents/<something> route
    above (performance, revenue, activity, wallet-portfolio, pnl-summary,
    escrow-compatibility, ...) still wins over this catch-all parameter.
    Adding a new literal /api/agents/* route BELOW this one would be
    silently shadowed by it.

    404s honestly when the id genuinely isn't in the store, rather than
    returning an empty shell the detail panel would render as blanks."""
    agent = await agent_store.get_agent_by_id(agent_id)
    if not agent:
