...
question: was that the only misclassification live on the site right
...
classification for every agent at once. This module is the fix
for that gap.

Real, scale finding that shaped this design: known_agents currently
holds 6,846 real, currently-`responding` agents (2026-08-28), across 6,287
DISTINCT service_endpoints (most, 6,253, used by exactly one real
agent each; the rest cluster mainly around two mass-registration
...
itself several HTTP round trips (2+ candidate URLs x 2 real
message-part shapes, plus retries on a transient failure), so a full
...
per call, progress persisted in Mongo, safe to run repeatedly on a
...
one or two batch runs, and they're exactly the agents most
...
distinct q402.quackai.ai agent shares the exact same endpoint),
so broad statistical confidence is cheap even before literal 100%
per-agent coverage is reached.

persistence, not just a live compute-on-view: results are written
directly onto each known_agents doc (escrow_compat_* fields below),
...
# single live check; an agent's protocol support is a structural
# property (see protocol_compat.py's own docstring), and re-auditing
# 6,000+ endpoints on a 24h cycle isn't a realistic cadence.
AUDIT_TTL_SECONDS = 7 * 24 * 60 * 60 # 7 days

# Real, confirmed-dominant mass-registration platforms (see
# docs/agent-interaction-patterns.md's own full census),
...
async def run_audit_batch(max_seconds: float = 60.0, limit: int | None = None) -> dict:
    ...
    running out of real, due agents). Returns a summary of what THIS
    ...
    # (the two mass platforms) start spending the real time budget.
    for phase_filter in (
    ...
    # Real, completion check, only true once a fresh count finds
    # zero due agents left in EITHER phase.
