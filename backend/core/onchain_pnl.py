...
DeFi agent, deliberately independent of whether a job ever went
through Tnega's own hire flow. reframing (2026-08-28) of the
...
independent on-chain execution history, trades/rebalances/deposits
...
Real, deliberate scope, same discipline as core/pnl.py:
  - Only Trading & DeFi category-group agents (core/category_groups.py),
    same reasoning: a content/identity agent has no portfolio
  ...
  - The real "operating wallet" checked is, in order: the agent's own
    ...
    Real, honest, live-checked finding (2026-08-28, 60 Trading & DeFi
    agents sampled across every non-"Trading Signals" sub-category):
    every single one had agent_wallet == owner_address. No agent in
    ...
    case a future agent's metadata ever does disclose one.
  - "activity" = real, Zerion-indexed BSC transactions that BOTH (a)
    ...
    the called contract to an known protocol.

    Real, important, live-caught correction (2026-08-28): (a) ALONE is
    NOT enough, confirmed live, several Trading & DeFi agents'
    wallets showed `mint`/`execute` transactions that were NOT DeFi
    at all: this project's OWN ERC-8004 identity-registry
    self-registration (`mint`, sent_to the registry contract every
    registered agent's owner wallet calls once), and calls to OTHER
    platforms' own generic escrow/action-router contracts (confirmed via
    a BscScan getsourcecode lookup: `TermixEscrow`,
    ...
    contract, none of them a DeFi protocol). None of those carried a
    Zerion `protocol_name`, Zerion's own classification
    ...
    `protocol_name` cut the real "has activity" rate on the 60-agent
    ...
    transaction types, but none of them is evidence a trade/
    rebalance/deposit/withdrawal actually executed.

Real, deliberate scope decision on raw event scanning: the ideal-sounding
approach floated was direct on-chain event scanning (e.g. PancakeSwap
...
only BSC RPC access is a single public node (core/rpc.py) with no
confirmed eth_getLogs support at the block-range depth a wallet's
full trading history would need, and BSCSCAN_API_KEY is but
...
own transaction-history API (already integrated, no block-range
limitation, operation_type classification) is the real, working
substitute used here, an indexed history, not raw logs, but and
live, with the same honest-failure discipline as the rest of this
project.

Real, attribution-confidence labeling, always surfaced:
  - `"high"`, the wallet checked is this agent's own owner_address, on
    record from its on-chain ERC-8004 registration, and the
    activity found is exactly what DeFi execution looks like (see
  ...
  - `"unconfirmed"`, reserved for a real, future case this module is
    ...
    registration owner. proof it's declared as this agent's wallet
...
window, honestly bounded: Zerion's own transaction list is
capped at 50 results per call and returns most-recent-first
(confirmed live), there is no real "this wallet's very first ever
transaction" API available to this project. So the window used here
is: the span from the OLDEST DeFi-execution transaction actually
returned in that most-recent page, to now, always labeled
"based on the most recent N transactions", NEVER claimed to be this
wallet's full history when more than 50 transactions exist.
"""
...
_MAX_POINT_GAP_MULTIPLIER = 2 # same real, tolerance discipline as core/pnl.py


def is_onchain_pnl_eligible(category: str | None) -> tuple[bool, str | None]:
    """Real, eligibility check, category only. Deliberately NO
    hire-flow requirement at all: that's the entire point of this
    reframing versus core/pnl.py."""
    group = category_groups.group_for_category(category)
    if group != PNL_ELIGIBLE_GROUP:
        return False, "Not a Trading & DeFi agent, on-chain performance isn't a coherent measure for this category of work."
    return True, None
...
async def _resolve_operating_wallet(owner_address: str, token_id, api_key: str | None) -> dict:
    ...
    confirmed live, every case checked so far, see module
    docstring)."""
...
def _pick_chart_period(window_seconds: float) -> str | None:
    for period, coverage in _CHART_PERIOD_COVERAGE_SECONDS.items():
        if window_seconds <= coverage * 0.9:
            return period
    return None # window wider than even "year", an honest "can't size a chart for this" outcome
...
async def get_historical_onchain_performance(
    ...
    real, result:
      ...
      - `applicable=True, has_activity=False`, right category, but no
        ...
        resolved wallet (an negative, not an error).
      - `applicable=True, has_activity=True`, evidence found; a
        real `pnl` sub-result when a value could be computed
    ...
    if not wallet:
        return {"applicable": True, "has_activity": False, "reason": "No owner/operating wallet on record for this agent."}
    ...
    if not activity.get("available"):
        return {
            ...
            "reason": f"on-chain activity check unavailable right now: {activity.get('reason')}",
        ...
        return {
            **base, "applicable": True, "has_activity": False,
            "reason": ("No real, Zerion-confirmed DeFi protocol execution (trades, deposits, withdrawals, LP "
                       ...
                       "operating wallet, in its most recent on-chain transaction history. (Other, "
    ...
    # corroborating evidence, best-effort, current DeFi
    # protocol positions (already the same data WalletPortfolioPanel
    # surfaces), never required for has_activity, just extra context.
    ...
    # dedicated, FIFO-cost-basis PnL endpoint, over the SAME window as
    ...
    # different methodologies (FIFO trade-matching here vs.
    # portfolio-value-at-two-points below), can legitimately disagree for a
    # wallet with in-window trading, and showing both, clearly
    # labeled, is more scientifically than silently picking one.
    ...
    # Real, best-effort PnL over that window, same methodology
    # as core/pnl.py (chart-based start/end value, minus gas), just
    # windowed by the agent's own activity instead of a Tnega job's
    # funding/delivery timestamps. A real, miss here still returns
    # has_activity=True with the evidence above, the PnL number is
    # a bonus, not a requirement for this signal to be worth showing.
    period = _pick_chart_period(window_seconds)
    if period is None:
        result["pnl"] = None
        result["pnl_reason"] = (f"This agent's DeFi activity spans {window_seconds / 86400:.0f} days, "
                                 f"wider than any chart period this project currently supports (up to 1 year).")
        return result

    chart = await zerion.get_wallet_chart(wallet, period)
    if not chart.get("available"):
        result["pnl"] = None
        result["pnl_reason"] = f"portfolio history unavailable from Zerion: {chart.get('reason')}"
        return result

    points = chart["points"]
    if len(points) < 2:
        result["pnl"] = None
        result["pnl_reason"] = "Not enough chart data points to compute a value change."
        return result
    ...
    if start_gap > max_gap or end_gap > max_gap:
        result["pnl"] = None
        result["pnl_reason"] = (f"The closest Zerion data points are too far from this window's real "
                                 f"start/end to trust (gaps: {start_gap / 60:.0f}m / {end_gap / 60:.0f}m "
                                 f"against this period's own real {point_spacing / 60:.0f}m resolution).")
        return result

    # gas cost, scoped to this window only (mined_at >=
    ...
    result["pnl"] = {
        ...
        "window_start_precision": ("the oldest DeFi-execution transaction actually found, capped at the "
                                    "50 most recent transactions on this wallet"),
    }
    return result
