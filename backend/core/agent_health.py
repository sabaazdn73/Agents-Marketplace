...
(needs 3+ settled jobs most agents never reach). Answers: "does this
agent's registered service actually respond right now", not "will it do a
good job" (stated honestly in the UI, not just here).

investigation before building this (2026-08-21), don't assume:
...
2. The source of truth is on-chain: the ERC-8004 identity registry's
   ...
   URI that decodes to JSON with a `services` array, the exact
   registration payload `bag erc8004 register --endpoint …` wrote.

3. format diversity confirmed by checking several OTHER real,
   ...
   gateway fetch, this app already uses https://ipfs.io/ipfs/ for
   agent image_urls, same gateway reused here), some are malformed
   junk (e.g. "ipfs://bort-v31-canary", not a CID), and some are
...
state model (four, not three), the extra state matters for honesty:
  - "responding", endpoint resolved AND it answered a HTTP
                        request (any status code counts; even a 404 proves
                        something is genuinely listening there, the goal
                        is "is anything running", not "is every route
                        correct").
  - "not_responding", endpoint resolved, but the HTTP request itself
  ...
  - "unknown", we could not even resolve the metadata (e.g. the
                        ...
                        be a false negative against agents that are
                        probably fine.
"""
...
_RESOLVE_RETRY_DELAY = 4.0 # backoff before retrying a failed resolution, see _resolve_services
_HEALTHCHECK_TIMEOUT = 4.0 # real, short, as specified, this is a liveness ping, not a job
# cold-start mitigation, see _check_one's own comment for the live
...
# budget, which would have produced a false negative on that exact
# check. 35s leaves headroom above the worst cold start actually
...
_CONCURRENCY = 12 # cap so a large agent list doesn't hammer IPFS/agent hosts at once
...
async def _multicall_tokenuris(
    ...
    """batched tokenURI() read for a chunk of agents, one eth_call,
    ...
    bytecode), and a stored agent on each resolves a real, distinct
    ...
    for tid, (success, ret) in zip(token_ids, results):
        ...
        try:
            ...
            continue # malformed return, treated as no URI, honestly skipped
    return out
...
async def _resolve_services(uri: str, client: httpx.AsyncClient) -> tuple[list[dict] | None, bool]:
    ...
    every agent then marked "unknown" directly, one at a time, right
    ...
    per-agent failure, a real, single-host transient failure under
    ...
    for attempt, delay in enumerate((0.0, _RESOLVE_RETRY_DELAY)):
        ...
        try:
            ...
            last_exc = e
            continue
    # resolution failure on BOTH attempts (gateway/host still down after
...
def _first_http_endpoint(services: list[dict] | None) -> str | None:
    """bug found and fixed 2026-08-25, via a cross-check against
    ...
    a large cluster of Termix-platform agents registers service
    ...
    That URL is and DOES answer (platform-backend.prod.termix.live is a
    real, live host), so our old "any HTTP response, even a 404, counts
...
async def _check_one(agent: dict, uri: str | None, client: httpx.AsyncClient, sem: asyncio.Semaphore) -> dict:
    ...
    async with sem:
        ...
        # with a longer timeout (this is exactly the wake-up-then-retry
        ...
        for attempt_timeout in (_HEALTHCHECK_TIMEOUT, _HEALTHCHECK_RETRY_TIMEOUT):
            try:
                ...
                # ANY HTTP response (including 4xx/5xx) proves something
                # is genuinely listening, see module docstring for why
                # that's the right bar for "is a service running here".
                return {
                    **now_iso_fields,
...
async def check_agents_health(agents: list[dict], limit: int | None = None) -> dict[str, dict]:
    """health-check pass over `agents` (each needs id/token_id at
    ...
    result, so the caller's merge leaves their existing data alone.
    ...
    once (e.g. after a refresh outage), a single call could try
    to probe thousands of endpoints at once. `limit` (optional, so
    every existing caller's behavior is unchanged) caps how many
    ...
    for aid, result in zip(tasks.keys(), results):
        if isinstance(result, Exception):
            print(f"[agent_health] check failed for {aid}: {result}")
            continue
        out[aid] = result
    return out
