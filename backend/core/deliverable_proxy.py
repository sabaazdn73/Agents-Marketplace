...
support, a OPTIONS preflight against it returns 405 with no
...
same gap erc8183_negotiate.py was built around (see that module's own
...
Fix: fetch the deliverable server-side (this module) and hand the bytes
+ content-type back to the browser from OUR OWN origin, which already
...
on-chain record points to, not just this one agent's, so it fixes the
same failure mode for any ERC-8183 seller without CORS support.

security note: unlike erc8183_negotiate.py's proxy (which only ever
...
buyer's behalf (SSRF) if unguarded. mitigations below: only http(s),
...
and a timeout. This is a best-effort DNS-resolve-time check, not a
...
_TIMEOUT = 15.0
_MAX_BYTES = 5 * 1024 * 1024 # 5MB, comfortably covers a deliverable (JSON/text/small image)
        ...
        async with httpx.AsyncClient(timeout=_TIMEOUT, follow_redirects=False) as client:
            async with client.stream("GET", url) as resp:
                if 300 <= resp.status_code < 400:
                    ...
                    # check above closes). A deliverable URL is never
                    # expected to redirect; treat it as a failure.
