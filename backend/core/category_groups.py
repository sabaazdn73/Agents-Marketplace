...
tradeoff already made for core/canary.py's own ALLOWED_GROUPS
scope, which used to keep a private, second, partial copy of this exact
mapping before this module existed).

fix (2026-08-28): this used to be duplicated, core/canary.py had
its own private `_GROUP_CATEGORIES` covering only 2 of the 5 groups
(canary's own scope never needed the rest). core/pnl.py needs the
...
'Unclassified' is deliberately not a key here, same reasoning as the
frontend's own file: it's not a classification, so it isn't a group.
"""
...
def group_for_category(category: str | None) -> str | None:
    """reverse lookup, the fine category's group id, or None
