...
Real, on-chain-balance-based Profit & Loss for a completed job,
the hiring wallet's own balance change, start vs. end, never a
creator-submitted or backtested number.

Real, deliberate simplification (2026-08-28): this used to require the
job's on-chain description to carry the "(Altana session)" hire-flow
marker, on the theory that only a session hire delegates real, ongoing
fund authority worth measuring. Real, finding that motivated
dropping that restriction: a live scan of 23,000 on-chain jobs found
**zero** Altana-session-funded jobs had ever happened, meaning this
feature had been completely dormant, structurally incapable of ever
showing a number, for its entire existence. Checked directly before
generalizing, not assumed: `job.client` is a real, directly-observable
wallet address for BOTH hire types, for a standard "Always Ask"
hire it's simply the wallet that called createJob/funded the job
(confirmed live against a job, #56654: `client` is a real, distinct
address, not some intermediate contract); for an Altana-session hire it's
the session/mini-wallet the agent was granted authority over. The
real, question this answers is the same either way: did the real
...
started with. Real, deliberately unchanged scope otherwise:
  - Only Trading & DeFi category-group agents (core/category_groups.py,
    mirroring frontend/src/categoryGroups.js), the only category
    ...
    fee isn't a PnL question, of course the buyer's balance drops
    by exactly the fee, that's just paying for a service, not trading.
  - Only a job that's genuinely been delivered (status SUBMITTED or
    COMPLETED), nothing to measure PnL over yet for an unfinished job.

methodology:
  1. The wallet tracked is job.client itself, the hiring
     wallet, whichever hire path was used. For a standard "Always
     Ask" hire, this is simply the wallet that called
     ...
     owner_address (a different, unrelated identity) either way.
  2. start/end timestamps: end = job.submittedAt (a real, exact
     on-chain field, the moment work concluded). Start has no
     on-chain field at all (confirmed absent from the job struct,
     the same gap frontend/src/jobTiming.js already documents in
     ...
     larger of the two hire paths' own default expiry buffers.
     Always labeled "approximate" here, the backend has no access to any
     browser's own precisely-recorded funding moment, so it can
     never honestly claim better than this tier.
  3. start/end portfolio VALUES: Zerion's real
     ...
     docs before assuming otherwise); returns a real, evenly-spaced
     series of points over a fixed period. The smallest period whose
     own coverage comfortably spans [start, end] is chosen, then the
     chart point closest to each target timestamp is used, its
     own timestamp is always reported alongside the target, and a
     real, tolerance (relative to that period's own measured
     point spacing, never a fixed guess) gates whether the match is close
     enough to trust at all.
  4. gas costs: Zerion's real /wallets/{address}/transactions/
     ...
     transaction in the [start, end] window. If any transaction
     in the window has no priced fee, the gas total is honestly
     flagged as a lower bound, not silently treated as exact.
  5. PnL = end_value - start_value - total_gas_usd.

Real, failure modes, every one reports a reason, never a
fabricated or silently-zeroed number. Two tiers, distinguished so the
UI can tell them apart: `applicable` (is this even the right kind of job
to have a PnL at all?) vs `available` (is a number computable RIGHT
NOW, given data gaps?).
"""
...
# Real, deliberate bound on how many of an agent's own recent jobs
# get PnL-checked per summary call, same real "recent_job_ids[:10]" cap
# core/agent_performance.py already applies for a related reason
# (bounded work per request). Each job here can cost up to 2
# Zerion calls (a chart + a transaction-fee lookup), so this also
# directly bounds Zerion quota use per agent-detail-page view.
MAX_JOBS_PER_SUMMARY = 8

# Real, on-chain-confirmed constants, matches frontend/src/jobTiming.js's
# own constants exactly (mirrored here since the backend can't import
# frontend JS; see that file for the full derivation of both).
DISPUTE_WINDOW_SECONDS = 604800       # 7 days, OptimisticPolicy.disputeWindow(), read live via eth_call
DEFAULT_EXPIRY_BUFFER_SECONDS = 65 * 60 # the larger of the two hire paths' own default expiry buffers

# Real, deliberate scope, the one category-group PnL is a coherent
# question for (see module docstring).
PNL_ELIGIBLE_GROUP = "trading-defi"

_DELIVERED_STATUSES = {"SUBMITTED", "COMPLETED"}

# chart periods this module will try, narrowest (best real
...
# Real, tolerance: how far the nearest chart point may sit
# from the target timestamp before this is reported as "too coarse
# to trust" instead of silently used, a multiple of the CHOSEN
# period's own actually-measured point spacing, never a fixed number
# unrelated to that period's resolution.
...
def is_pnl_eligible(category: str | None, description: str | None = None) -> tuple[bool, str | None]:
    ...
    the only gate, see module docstring for why the Altana-session
    restriction was dropped."""
    group = category_groups.group_for_category(category)
    if group != PNL_ELIGIBLE_GROUP:
        return False, "Not a Trading & DeFi agent, PnL isn't a coherent measure for this category of work."
    return True, None


def _approximate_funded_at(expired_at: int) -> int:
    """Real, deterministic, start-time estimate, the backend has
    no access to any browser's own precisely-recorded funding moment
    ...
    only estimate computable from on-chain data alone."""
...
def _pick_chart_period(window_seconds: float) -> str | None:
    """Real, smallest chart period whose own coverage comfortably
    spans the real [start, end] window, for the best resolution
    available. None if the window exceeds every period this project
    ...
    for period, coverage in _CHART_PERIOD_COVERAGE_SECONDS.items():
        if window_seconds <= coverage * 0.9: # margin so the window doesn't sit right at the period's own edge
            return period
    return None


def _nearest_point(points: list[tuple[float, float]], target_ts: float) -> tuple[float, float] | None:
    """nearest [timestamp, value] to a target, never
    interpolated or fabricated, always an point Zerion returned."""
    if not points:
        return None
...
async def compute_job_pnl(job_id: int, category: str | None) -> dict:
    ...
    result, either a number with full provenance, or a
    ...
    scope (wrong category, not an Altana-session hire), the UI
    should show nothing, not an error. `available=False` (with
    `applicable=True`) means it's the RIGHT kind of job but a data
    gap or timing issue means a number can't be computed right
    now, the UI should say so honestly."""
    job = await get_job(job_id)
    if job is None:
        return {"available": False, "applicable": None, "reason": f"Couldn't read job #{job_id} on-chain."}
    ...
    if job["status"] not in _DELIVERED_STATUSES:
        return {"available": False, "applicable": True,
                "reason": f"This job hasn't been delivered yet (on-chain status: {job['status']})."}
    ...
    if start_ts >= end_ts:
        return {"available": False, "applicable": True,
                "reason": "The real, computed start-time estimate isn't chronologically sane for this job, "
                          "can't establish a window to measure."}

    window_seconds = end_ts - start_ts
    period = _pick_chart_period(window_seconds)
    if period is None:
        return {"available": False, "applicable": True,
                "reason": f"This job's window ({window_seconds / 86400:.1f} days) is wider than any "
                          f"chart period this project currently supports."}

    chart = await zerion.get_wallet_chart(wallet, period)
    if not chart.get("available"):
        return {"available": False, "applicable": True,
                "reason": f"portfolio history unavailable from Zerion: {chart.get('reason')}"}

    points = chart["points"]
    start_point = _nearest_point(points, start_ts)
    end_point = _nearest_point(points, end_ts)
    if start_point is None or end_point is None:
        return {"available": False, "applicable": True, "reason": "No chart data points in this job's window."}

    # Real, MEASURED point spacing (not estimated from period/count), the
    # basis for "is the nearest point close enough to trust".
    ...
    if start_gap > max_gap or end_gap > max_gap:
        return {
            ...
            "reason": f"The closest Zerion data points are too far from this job's start/end to "
                      f"trust (gaps: {start_gap / 60:.0f}m / {end_gap / 60:.0f}m against this period's "
    ...
    if activity.get("available"):
        for tx in activity.get("transactions", []):
            ...
            if isinstance(fee, (int, float)):
                ...
                # A transaction in the window with no priced fee,
                # the gas total below is an LOWER bound from
...
async def compute_agent_pnl_summary(owner_address: str, category: str | None) -> dict:
    ...
    job's PnL is but not very meaningful on its own; the real,
    question a buyer asks is "when people have actually hired
    this agent, did their own money come out ahead or behind").

    Real, short-circuit: checks category eligibility ONCE, up
    front, never spends Zerion quota probing individual jobs for an
    agent whose category makes this fundamentally inapplicable."""
    group = category_groups.group_for_category(category)
    if group != PNL_ELIGIBLE_GROUP:
        return {"applicable": False, "jobs": [],
                "reason": "Not a Trading & DeFi agent, PnL isn't a coherent measure for this category of work."}
    ...
    if not job_ids:
        return {"applicable": True, "jobs": [], "total_pnl_usd": None,
                "reason": "No on-chain jobs found for this agent yet."}

    checked_ids = job_ids[:MAX_JOBS_PER_SUMMARY]
    results = await asyncio.gather(*(compute_job_pnl(jid, category) for jid in checked_ids))

    # Real, split: only jobs that were BOTH applicable (a
    # delivered Trading & DeFi hire) AND successfully computed count
    # toward the total, a job that's applicable but data-gapped is
    # surfaced (so the UI can be about a real, partial picture),
    ...
    if not eligible_jobs:
        return {"applicable": True, "jobs": [], "total_pnl_usd": None,
                "reason": f"None of this agent's last {len(checked_ids)} checked jobs have been delivered "
