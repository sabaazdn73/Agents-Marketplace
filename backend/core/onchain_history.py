...
the v2 Etherscan-unified API; the old dedicated api.bscscan.com host
...
, already integrated, already proven live on BSC. Real, caveat this
carries forward: Zerion is also an indexer, not literally raw chain data
, the meaningful, distinction this feature still delivers is
...
(unlike onchain_pnl.py's DeFi-execution-only probe), the point here
...
async def get_full_onchain_history(owner_address: str) -> dict:
    ...
    a transaction list plus a summary (protocols seen, token
