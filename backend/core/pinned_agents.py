...
genuinely and registered on BSC mainnet, confirmed live via a real
...
The only fix for an agent we specifically know is genuine: read it
...
trade-off, stated honestly: because 8004scan never indexed this agent,
        ...
        for token_id in PINNED_TOKEN_IDS:
            try:
                ...
                if not owner or not uri:
                    print(f"[pinned_agents] token {token_id}: no owner/tokenURI on-chain, skipping")
                    continue
                ...
                out.append({
                    ...
                    "chain_id": 56,
                    # gap: 8004scan never indexed this agent (see
