...
O(n²) comparison across tens of thousands of agents) is a real,
...
running several real, different agents, different names, different
descriptions, different purposes, must NOT be collapsed just
because they share an owner. Only template/boilerplate
duplication (the problem the original heuristic was trying to catch)
should be.

design: two agents are only actually treated as duplicates of each
other if BOTH of these hold:
  1. Their description-template signature matches (the blocking key
     , necessary, not sufficient on its own).
  2. At least one further, real, corroborating signal also agrees:
       - the exact same registered service endpoint (strong signal,
         same underlying deployment), OR
       - registration timestamps within a tight window of each other
         (same registration burst, a mass-registration script
...
agent), the real, deliberate fix for the exact case the user
...
# registration-burst window, agents created within this many real
# seconds of each other are treated as a real, corroborating "same
# campaign" signal. 10 minutes is generous enough to catch a batch
# script's own real, sequential on-chain registrations (each a separate
# tx, so not instantaneous) without being so wide it starts
...
def _cluster_signature(name: str, description: str) -> tuple:
    ...
    avoid a circular import, aggregate.py will import FROM this
    module instead) so this stays the single, source of truth for it
    going forward."""
...
def cluster_agents(agents: list[dict]) -> dict[int, int]:
    ...
    perf fix (2026-08-27, found while investigating a real, live
    ...
    measured on the current known_agents store (10,837 agents): the
    single largest bucket alone has 1,591 agents, needing ~1.26M pairwise
    calls by itself, and the total across all buckets was ~2.52M
    ...
    of the three corroborating signals in the old _corroborates() is
    ...
    group-by (union everyone sharing the same value), no pairwise
    ...
    chain of in-window neighbors, unioning each adjacent link
    ...
    either, since sorted order means their gap is only larger. Same
    ...
    efficiency note (unchanged): the blocking step (grouping by
    description-template signature first) is still what keeps the overall
    cost manageable, only agents that already share a description
    ...
    # not an ad-hoc bucket dict, so a chain of corroborated agents
    ...
    for sig, idxs in buckets.items():
        ...
        # 1) Same registered service endpoint, exact-match signal,
        ...
        # 2) registration-burst proximity, sort by timestamp, union
        # only adjacent pairs within the window (see docstring above
...
def diversify(agents: list[dict], per_cluster_cap: int) -> list[dict]:
    ...
    contract (keep at most `per_cluster_cap` per cluster,
    preserving order), but backed by the multi-signal clustering
    ...
    owner independently running two different agents built from the
    same boilerplate, with no other corroborating signal) are correctly
    NOT collapsed under this rule, each keeps its own slot."""
