...
THAT module's purpose (instant marketplace page loads), but a real,
confirmed bug for THIS one. Live-checked before fixing: job_counter =
56,665 jobs total on the shared contract; WINDOW = 1,500, only the
most recent 2.6% of all job ids were ever visible, so any agent with
...
job id from 1 to the current job_counter, backfilled once and kept
current via a bounded re-check pass. See job_index.py's own docstring and
docs/verification-methodology.md for the full investigation and
before/after numbers.

Real, completeness signal, always included: `index_completeness`
...
Real, deliberate scope on which jobs count as "earned":
  - SUBMITTED and COMPLETED, the same pair core/pnl.py's own
    _DELIVERED_STATUSES already uses, and the same reasoning
    ...
    earning has happened) and REJECTED/EXPIRED (no payment reached
    the provider).
  - The amount counted per job is that job's own on-chain `budget`
    field, the amount actually locked and (for a delivered job)
...
currency handling: the ERC-8183 settlement token is read LIVE
...
already applies to `decimals()` before every hire), then that
...
Real, zero-state: an agent with no SUBMITTED/COMPLETED jobs
in the scanned window returns `has_earnings: False` with a clear,
specific reason (never hidden, never a fabricated $0 chart).
"""
...
# Real, long TTL, the settlement token's own identity (which token,
...
# data (e.g. core/protocol_compat.py's own real 24h probe cache).
...
async def _resolve_payment_token() -> dict | None:
    """Real, live read of the ERC-8183 settlement token's identity
    ...
    only on a RPC failure; cached 24h once data is
    obtained."""
    ...
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            ...
            if not token_result:
                return _token_cache # real, fallback to whatever we last had, if anything
        ...
        return _token_cache # real, honest, keep serving the last value we had, if any
...
async def get_revenue_timeline(owner_address: str) -> dict:
    """The real, main entry point. Always returns a real, result,
    either a cumulative-earnings timeline with full provenance, or a
    clear, specific reason there isn't one yet. Never a fabricated number.

    Shape:
      - `has_earnings`: False when there's nothing to show (no jobs
      ...
      - `timeline`: real, chronological (oldest first) per-job entries,
        job id, ISO date (from the job's own submittedAt),
    ...
    if token is None:
        return {
            "has_earnings": False,
            "reason": "Found real, delivered jobs, but couldn't read the settlement token's identity from "
                      "the chain right now, try again shortly.",
            **completeness,
        }

    decimals = token["decimals"]
    # Real, chronological order, submittedAt is the on-chain moment
    # each job was actually delivered; falls back to job id (also
    # monotonic with creation order) for the rare case
